...
func setupRoutes(r *gin.Engine, h *handlers.Handler) {
	...
	// WebSocket progress channel (compatibility alias)
	r.GET("/ws/progress", h.WSProgress)

	// 静态文件服务（带缓存，支持子目录，便于浏览器预览模型）
...
func rateLimitMiddleware(cfg *config.Config) gin.HandlerFunc {
	...
	disabled := cfg != nil && cfg.RateLimitPerMinute == 0
	...
	return func(c *gin.Context) {
		...
		clientIP := c.ClientIP()
		// Exempt loopback / private addresses (local workbench + e2e).
		...
		if !exists || now.After(info.resetTime) {
			clients[clientIP] = &clientInfo{count: 1, resetTime: now.Add(1 * time.Minute)}
			mu.Unlock()
			c.Next()
			return
		}
		if info.count >= limit {
			...
			c.AbortWithStatusJSON(http.StatusTooManyRequests, gin.H{
				"error":       "请求过于频繁，请稍后再试",
				"retry_after": int(reset.Sub(now).Seconds()),
			})
			return
		}
		info.count++
